  ...
  test('Air Request - Step 2 - 9# and #10 Scenario - Add Additional Passenger', async ({
    ...
    await test.step('5 - Click on Start from scratch', async () => {
      ...
      await test.step('9# Add first passenger', async () => {
        try {
          passengerName =
            (await page.locator(airRequest.NAMES_FOR_AVAILABLE_CHECKBOXES).first().textContent()) ??
            'Not found';
          ...
          const phoneUI = await page.locator(airRequest.PHONE_INPUT_PASSENGER(1)).inputValue();
          await expect(page.locator(airRequest.PASSENGER_EMAIL(1))).toHaveValue(
            addedPassenger.email,
          );
          const firstName = await page.locator(airRequest.FIRST_NAME_PASSENGER(1)).inputValue();
          const lastName = await page.locator(airRequest.LAST_NAME_PASSENGER(1)).inputValue();
          ...
          console.error('Secondary Passenger non matching pre populated fields');
      ...
      await test.step('9# - ER - Gender has red asterisk and its mandatory', async () => {
        await expect(page.locator(airRequest.GENDER_RED_ASTERISK).last()).toBeVisible();
      ...
      await test.step('9# - ER Secondary Passenger DoB', async () => {
        try {
          ...
          await expect(page.locator(airRequest.DOB_YEAR(2))).not.toHaveClass(/border-red/i);
        } catch (err) {
      ...
      await test.step('9# - ER  - Add passport information toggle OFF by default', async () => {
        await expect(page.locator(airRequest.ADD_PASSPORT_INFORMATION(2)).last()).toHaveAttribute(
          'aria-checked',
          'false',
        );
      });
      ...
      await test.step('9# - ER  - Edit Secondary Passenger email', async () => {
        try {
          ...
          await expect(page.locator(airRequest.PASSENGER_EMAIL(1))).toHaveClass(/border-red/i);
          await airRequest.fillPassengerEmail('', 2);
          await airRequest.clickLabel();
          await expect(page.locator(airRequest.WARNING_EMAIL)).not.toBeVisible();
      ...
      await test.step('10# - ER  - Add Additional Passenger passport - Verify if user try to upload file format that isn’t allowed the validation appear', async () => {
        try {
          await expect(page.locator(airRequest.ADD_PASSPORT_INFORMATION(2))).toBeVisible();
      ...
      await test.step('11# - ER  - Edit Additional Passenger first name', async () => {
        try {
          ...
          await expect(page.locator(airRequest.FIRST_NAME_PASSENGER(1))).toHaveClass(/border-red/i);
      ...
      await test.step('13# Scenario - Edit Secondary Passenger Last Name', async () => {
        try {
          await airRequest.clearPassengerLastName(1);
          await airRequest.clickLabel();
          await expect(page.locator(airRequest.WARNING_LAST_NAME)).toContainText(
            'Last name is required',
          );
          await expect(page.locator(airRequest.LAST_NAME_PASSENGER(1))).toHaveClass(/border-red/i);
      ...
      await test.step('15# Scenario - Edit Secondary Passenger email', async () => {
        try {
          ...
          await airRequest.clickLabel();
          await expect(page.locator(airRequest.WARNING_EMAIL)).not.toBeVisible();
          await expect(page.locator(airRequest.PASSENGER_EMAIL(1))).not.toHaveClass(/border-red/i);
          await airRequest.fillPassengerEmail(EMAIL, 2);
        } catch (err) {
          console.error('Edit Secondary Passenger email');
          test.info().errors.push({
      ...
      await test.step('17# Scenario - Remove Additional Passenger', async () => {
        try {
          await expect(page.locator(airRequest.HEADER_H4).last()).toContainText('Passenger 2');
          await airRequest.deleteTraveler();
          await expect(page.locator(airRequest.HEADER_H4).last()).toContainText('Passenger 1');
          await airRequest.addAdditionalPassenger();
          await expect(page.locator(airRequest.POP_UP_DIALOG)).toBeVisible();
          await expect(page.locator(airRequest.POP_UP_CANCEL)).toBeVisible();
          await expect(page.locator(airRequest.POP_UP_ADD_PASSENGERS)).toBeVisible();
