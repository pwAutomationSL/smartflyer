...
const DAY = '7';
const YEAR = '1993';
test.describe('AR-003 - Air Request - Step #18 ', () => {
  test.setTimeout(200_000);
  test('Air Request - Step 2 - 18# Scenario - Add new traveler, don’t save to client profile', async ({
    loginPage,
    username,
    password,
    page,
    ...
    await test.step('5 - Click on Start from scratch', async () => {
      ...
      await test.step('18# Add new traveler, don’t save to client profile', async () => {
      ...
      await test.step('18# Add client First name, Last name, Gender, Date of birth (all fields required)', async () => {
        ...
        await expect(page.locator(airRequest.LAST_NAME_PASSENGER(1))).toHaveValue(
          PASSENGER_LAST_NAME,
        );
        await expect(
          page
