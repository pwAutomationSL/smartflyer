...
test.describe('AR-004 - Air Request - Step 3', () => {
  test('Air Request - Step 3 -3# Scenario - Multi-city trip type', async ({
    ...
    await test.step('11 - Add information for Flight 1: Departure airport, Arrival airport, Departure date, Departure time ', async () => {
      await airRequest.selectDepartureAirport2LettersFlight1(DEPARTURESHORT_2LETTERS);
      ...
      await airRequest.selectArrivalAirport2LettersFligh1(ARRIVAL_SHORT_2LETTERS);
      await expect(page.locator(airRequest.AIRPORTS_VALID_RESULTS).first()).toBeVisible();
      ...
      const searchResultsArribal = await page.locator(airRequest.AIRPORTS_VALID_RESULTS).count();
    ...
    await test.step('12 - Add information for Flight 2: Departure airport, Arrival airport, Departure date, Departure time   ', async () => {
      await airRequest.selectDepartureAirport2LettersFlight2(DEPARTURESHORT_2LETTERS);
      ...
      const searchResults = await page.locator(airRequest.AIRPORTS_VALID_RESULTS).count();
      expect(searchResults).toBeGreaterThan(1);
      await airRequest.selectArrivalAirport2LettersFligh2(ARRIVAL_SHORT_2LETTERS);
      await expect(page.locator(airRequest.AIRPORTS_VALID_RESULTS).first()).toBeVisible();
      const cityCountryArribal = await page
        .locator(airRequest.CITY_COUNTRY_RESULT)
        .first()
