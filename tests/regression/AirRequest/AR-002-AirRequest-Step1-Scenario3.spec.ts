...
test.describe('AR-002 - Air Request - Step 1', () => {
  test('Scenario 3 - Admin → Continue from Draft', async ({
    ...
    await test.step('Search the client and go to the client page, create data to use from Draft', async () => {
      ...
      await expect(page.locator(airRequest.PHONE_INPUT).inputValue()).resolves.toBe(
        getPrefilledData.phone,
      );
      await expect(page.locator(airRequest.AGENT_SELECT)).toContainText(getPrefilledData.agent);
      await expect(page.locator(airRequest.EMAIL_INPUT(1)).inputValue()).resolves.toBe(
        getPrefilledData.email,
      );
    });
  });
});
