    ...
    await test.step("5 - Click on Start from scratch", async () => {
      ...
      await test.step("5# Scenario - Edit Primary Passenger DoB", async () => {
        try {
          ...
          await expect(page.locator(airRequest.DOB_DAY(1))).toHaveClass(
            /border-red/i
          );
          await expect(page.locator(airRequest.DOB_YEAR(1))).toHaveClass(
            /border-red/i
          );
          ...
          await airRequest.clickLabel();
          await expect(page.locator(airRequest.DOB_DAY(1))).not.toHaveClass(
            /border-red/i
          );
          await expect(page.locator(airRequest.DOB_YEAR(1))).not.toHaveClass(
            /border-red/i
          );
        } catch (err) {
