...
test.describe('AR-007 - Air Request - E2E - 2 - One Passenger, Round Trip flight', () => {
  test('Air Request - 2 -One Passenger, Round Trip flight', async ({
    ...
    await test.step('10 - Review', async () => {
      ...
      await expect(page.locator(airRequest.DEPARTURE_FROM).last()).toContainText(ARRIVAL);
      await expect(page.locator(airRequest.ARRIVE_AT).first()).toContainText(ARRIVAL);
      await expect(page.locator(airRequest.ARRIVE_AT).last()).toContainText(DEPARTURE);
