...
test.describe('PF-005 - e2e for Partner Form - Cruise Line', () => {
  test('Login at Society (env) as an Admin', async ({
    ...
    await test.step('7 - Overview', async () => {
      await partnersOnsite.oneSentenceToDescribe();
      await partners.destinationsCoverage();
      await partners.cruiseELevate();
      await partners.cruisePreferredPartner();
      await partners.virtuosoAmenities();
    ...
    await test.step('9 - Contacts', async () => {
      ...
      await partnersHotel.fillKeyAccountContact();
      await partnersHotel.fillAdditionalContactCruise();
    });
    ...
    await test.step('11 - Things to Know', async () => {
      await partnersOnsite.fillIndustryAffiliationTTKCruise();
      await partnersOnsite.averageItineraryCruise();
    });
    ...
    await test.step('13 - Booking and Commissions', async () => {
      await partners.checkCommissionsOfferingCruise();
      await partnersOnsite.checkCommissionsOfferingHotel();
      await partnersOnsite.bestWayToBookCruise();
      await partnersOnsite.commissionPaymentDetailsCruise();
    });
    ...
    await test.step('15 - Latest updates', async () => {
      await partnersOnsite.latestUpdatesMarketingCruise();
      await partnersOnsite.offersAndPromotionsCruise();
      await partnersOnsite.trainingCruise();
      await partnersOnsite.marketingMKTCruise();
      await partnersHotel.socialMediaStoryHotelMKCruise();
    });
