...
test.describe('PF-005 - e2e for Partner Form', () => {
  test('Login at Society (env) as an Admin', async ({
    ...
    await test.step('11 - Click Save and Next and Land on Things To Know Tab', async () => {
      await partners.saveAndNext();
      await expect(page.locator(partnersHotel.THINGS_TO_KNOW_TAB_TOP)).toHaveAttribute(
        'aria-selected',
        'true',
      );
      await expect(page.locator(partnersHotel.INFO_TEXT_THINGS_TO_KNOW_TOP)).toHaveText(
    ...
    await test.step('15 - Click Save and Next and Land on Latest Updates', async () => {
      await partners.saveAndNext();
      await expect(page.locator(partnersHotel.LATESTS_UPDATES_TAB_TOP)).toHaveAttribute(
        'aria-selected',
        'true',
      );
    });
