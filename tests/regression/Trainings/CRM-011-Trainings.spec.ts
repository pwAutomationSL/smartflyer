...
test.describe("CRM-011 Trainings", () => {
  test("As an admin, i want to create a new training", async ({
    ...
    await test.step("Add new Training", async () => {
      ...
      await expect(page.locator(trainings.SUCCESS_POPUP)).toContainText(
        "Success"
      );
    ...
    await test.step("Search the added  Trainingand", async () => {
      ...
      await expect(page.locator(trainings.RESULT_TITLE)).toContainText(
        TRAINING_NAME
      );
      await expect(page.locator(trainings.RESULT_DESCRIPTION)).toContainText(
        shortDescription
      );
      await expect(page.locator(trainings.RESULT_VIDEO)).toHaveAttribute(
        "data-url",
        video
      );
    });
    await test.step("Edit the added  Training and verify changes are present", async () => {
      ...
      await trainings.searchTraining(TRAINING_NAME);
      await expect(page.locator(trainings.RESULT_TITLE)).not.toContainText(
        TRAINING_NAME_UPDATE
      );
      await expect(
        page.locator(trainings.RESULT_DESCRIPTION)
      ).not.toContainText(shortDescription_update);
    });
