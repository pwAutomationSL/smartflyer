...
test.describe("GS-001 - Search - Clients filter", () => {
  test("Login at Society (env) as an Admin Search by Last Name", async ({
    ...
    await test.step("3 - Assert new post is visible in forum", async () => {
      await expect(page.locator(clients.USERNAME_HEADER).first()).toContainText(
        LAST_NAME
      );
    });
    ...
    await test.step("7 - Assert result is 1, title and details are correct", async () => {
      ...
      expect(count).toBe(1);
      await expect(
        page.locator(searchPage.SEARCH_RESULT_MATCHES_DETAIL).first()
      ).toContainText(LAST_NAME);
      await expect(
        page.locator(searchPage.SEARCH_RESULT_MATCHES_DETAIL).last()
      ).toContainText(EMAIL);
