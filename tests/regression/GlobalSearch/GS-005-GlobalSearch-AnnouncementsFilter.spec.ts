...
test.describe('GS-005 - Search - Announcements filter', () => {
  test('Login at Society (env) as an Admin', async ({
    ...
    await test.step('3 - Assert new post is visible in forum', async () => {
      await expect(page.locator(forum.TITLE_NEW_POST).first()).toContainText(TITLE);
      await expect(page.locator(forum.DETAILS_POST).first()).toContainText(DETAILS);
    });
    ...
    await test.step('5 - Assert result is 1, title and details are correct', async () => {
      const announcement = searchPage.resultContaining(TITLE);

      await expect(announcement).toBeVisible();
      await expect(announcement).toContainText(TITLE);
      await expect(announcement).toContainText(DETAILS);
    });
  });
});
