...
const PROPERTY_NAME = "Brand " + uniqueId();
let json: any;

test.describe("GS-002 - Search - Partners filter Brand", () => {
  test("Login at Society (env) as an Admin", async ({
    ...
    await test.step("6 - Verify the header text.", async () => {
      ...
      await expect(page.locator(partners.DRAFT_BUTTON_FIRST)).toContainText(
        "Approved",
        {
          timeout: 25000,
        }
